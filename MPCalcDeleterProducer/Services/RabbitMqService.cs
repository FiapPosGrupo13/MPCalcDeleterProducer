...
public class RabbitMqService : IRabbitMqService
{
    ...
    {
        _routingKey = configuration["RabbitMQ:RoutingKey"] ?? "default_routing_key";
        ...
        _dlqName = configuration["RabbitMQ:DLQName"] ?? $"{_queueName}.error"; // Ex.: mpcalc-Deleter-queue-dlq
    ...
    public async Task PublishMessageAsync(string message)
    {
        ...
        await channel.QueueBindAsync(
            queue: _dlqName,
            exchange: "topic_exchange",
            routingKey: "contact.remove.*.error");
